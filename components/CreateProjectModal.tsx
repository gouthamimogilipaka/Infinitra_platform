...
export const CreateProjectModal: React.FC<CreateProjectModalProps> = ({ isOpen, onClose }) => {
    ...
    // FIX: Add state for deadline and duration to capture all required project fields.
    const [deadline, setDeadline] = useState('');
    const [duration, setDuration] = useState('');

    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        // FIX: Add deadline and duration to the form validation check.
        if (!title || !description || !value || !currentUser || !deadline || !duration) return;

        const projectValue = parseFloat(value);
        // FIX: Add deadline and estimated_duration_days to the new project object to match the 'Project' type.
        ...
        // FIX: Reset deadline and duration fields after submission.
        setDeadline('');
        setDuration('');
    };
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center backdrop-blur-sm" onClick={onClose}>
            <div className="bg-gray-800 rounded-xl shadow-2xl p-8 w-full max-w-lg border border-gray-700 relative" onClick={e => e.stopPropagation()}>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    {/* FIX: Restructure form layout and add inputs for deadline and duration. */}
